...
def _normalise_user_coins(data: dict) -> bool:
    """Ensure every expected key exists on a user coins record. Returns True if
    anything was changed so the caller can decide whether to save."""
    changed = False

    for key, value in _default_user_coins().items():
        if key not in data:
            data[key] = value
            changed = True
    ...
    if not isinstance(data.get("star_meta"), dict):
        data["star_meta"] = {"day": _today_key(), "given": {}}
        changed = True
...
def ensure_user_coins(user_id):
    """Load coins, ensure the user is present and well-formed, persist only if
    anything changed. Returns the full coins dict."""
    uid = str(user_id)
    coins = load_coins()

    if uid not in coins:
        coins[uid] = _default_user_coins()
        save_coins(coins)
        return coins

    if _normalise_user_coins(coins[uid]):
        save_coins(coins)
...
class Listeners(commands.Cog):
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, user: discord.User):
        ...
        # Single load → mutate both users → single save. The previous version
        # called load_coins three times with ensure_user_coins in between,
        # which created a race where concurrent reactions could clobber each
        # other's writes.
        giver_id = str(user.id)
        receiver_id = str(message.author.id)

        coins = load_coins()
        coins.setdefault(giver_id, _default_user_coins())
        coins.setdefault(receiver_id, _default_user_coins())
        _normalise_user_coins(coins[giver_id])
        _normalise_user_coins(coins[receiver_id])

        giver = coins[giver_id]
        receiver = coins[receiver_id]

        given_today = int(giver["star_meta"]["given"].get(receiver_id, 0))
        if given_today >= 2:
            return

        giver["star_meta"]["given"][receiver_id] = given_today + 1
