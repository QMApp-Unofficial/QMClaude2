...
from ui_utils import C, embed, error, warn, success, ConfirmView


async def _confirm(ctx, *, title: str, description: str, timeout: float = 30.0) -> bool:
    """Shared destructive-action confirmation. Returns True only if the
    invoker clicks Confirm before the view times out."""
    view = ConfirmView(ctx.author.id, timeout=timeout)
    prompt = embed(f"⚠️  {title}", description, C.WARN,
    ...
    if view.value is not True:
        try:
            await view.message.edit(
                embed=embed("❌  Cancelled", "No changes were made.", C.NEUTRAL),
                view=view,
            )
...
class ModTools(commands.Cog):
    ...
    async def nuke(self, ctx, *, reason: str = "Nuked"):
        ...
        if not await _confirm(
            ...
            title="Confirm Nuke",
            description=(
                f"This will **delete and recreate** {ch.mention}.\n"
                "All messages, pins, and webhooks tied to this channel will be lost.\n\n"
                f"Reason: _{reason}_"
            ),
        ):
            return
    ...
    async def massrole(self, ctx, action: str, role: discord.Role):
        ...
        non_bot_count = sum(1 for m in ctx.guild.members if not m.bot)
        verb = "add to" if action == "add" else "remove from"
        if not await _confirm(
            ctx,
            title="Confirm Mass Role",
            description=(
                f"This will **{action}** the role {role.mention} "
                f"{'on' if action == 'add' else 'from'} up to **{non_bot_count}** members.\n"
                "This can't be undone automatically and may take a while."
            ),
        ):
            return
    ...
    async def restorejson(self, ctx):
        ...
        if not await _confirm(
            ctx,
            title="Confirm JSON Restore",
            description=(
                f"This will overwrite live JSON files in `{DATA_PATH}` with the contents of "
                f"`{attachment.filename}`.\n"
                "The current live JSON files will be copied to `pre_restore_backup/` first, but "
                "this action is hard to reverse once other systems read the new state.\n\n"
                "Only proceed if you intend to roll back to the uploaded backup."
            ),
            timeout=45.0,
        ):
            return
