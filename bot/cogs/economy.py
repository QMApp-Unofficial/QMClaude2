...
# ─── Career Pick View (dropdown) ──────────────────────────────────────────────

class _CareerSelect(discord.ui.Select):
        ...
        for key, data in CAREER_FIELDS.items():
            ...
            options.append(
                discord.SelectOption(
                    label=data["name"],
                    description=f"Start as {starter['title']} · {starter['min']}–{starter['max']} coins/shift"[:100],
                    value=key,
                    emoji=data["icon"],
                )
            )
        super().__init__(
            placeholder="Pick a career field — this is permanent",
            min_values=1,
            max_values=1,
            options=options,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if user.get("career_field"):
            return await interaction.response.send_message(
                embed=warn(
                    "Career",
                    f"You already work in **{CAREER_FIELDS[user['career_field']]['name']}**. "
        ...
        e = success(
            ...
            "Use `/work` every hour to earn coins and rack up shifts.\n"
            "**10 shifts** earns your first promotion.",
        )
...
class CareerPickView(discord.ui.View):
    def __init__(self, author_id: int):
        ...
        self.author_id = author_id
        self.add_item(_CareerSelect(author_id))
...
class Economy(commands.Cog):
    ...
    async def reseteconomy(self, ctx):
        confirm = ConfirmView(ctx.author.id, timeout=30)
        prompt = embed(
            f"{E.WARN_ACT}  Confirm Economy Reset",
            "**This is irreversible.**\n\n"
            "All wallets will be reset to **100** coins and all banks and debts cleared.\n"
            "Trivia prizes will also be reduced by **75% for 24 hours**.\n\n"
            "Press **Confirm** within 30 seconds to proceed.",
            C.WARN,
            footer=f"Requested by {ctx.author.display_name}",
        )
        confirm.message = await ctx.send(embed=prompt, view=confirm)
        await confirm.wait()

        if confirm.value is not True:
            cancelled = embed(
                f"{E.CROSS}  Cancelled",
                "No changes were made to the economy.",
                C.NEUTRAL,
            )
            try:
                await confirm.message.edit(embed=cancelled, view=confirm)
        ...
        try:
            await confirm.message.edit(embed=e, view=confirm)
        except discord.HTTPException:
            await ctx.send(embed=e)
