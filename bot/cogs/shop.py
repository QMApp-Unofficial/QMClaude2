...
# =========================
# Shop / claim select menus
# =========================

def _buy_item(user_id: int, item: str, *, star_shop: bool) -> tuple[bool, str]:
    """Shared buy logic for coin and star shops.

    Returns (ok, message). Caller is responsible for formatting / sending.
    """
    price_table = STAR_SHOP_ITEMS if star_shop else COIN_SHOP_ITEMS
    stock_key = "star_shop" if star_shop else "coin_shop"
    currency = "stars" if star_shop else "wallet"
    symbol = "✦" if star_shop else "¢"

    if item not in price_table:
        return False, "Item not found."

    stock = ensure_shop_stock(load_shop_stock())
    qty = int(stock[stock_key].get(item, 0))
    if qty <= 0:
        return False, "That item is out of stock."

    price = int(price_table[item]["price"])
    coins = load_coins()
    user = ensure_user(coins, user_id)

    if int(user.get(currency, 0)) < price:
        return False, "Not enough " + ("stars." if star_shop else "coins.")

    inv = load_inventory()
    user_inv = ensure_inventory(inv, user_id)

    user[currency] = int(user[currency]) - price
    user_inv[item] = int(user_inv.get(item, 0)) + 1
    stock[stock_key][item] = qty - 1

    save_coins(coins)
    save_inventory(inv)
    save_shop_stock(stock)

    remaining = int(stock[stock_key][item])
    balance = int(user[currency])
    return True, (
        f"Bought **{item}** for `{price}` {symbol}.\n"
        f"Stock left: `{remaining}`  ·  Your {currency}: `{balance}` {symbol}"
    )


class _BuyShopSelect(discord.ui.Select):
    def __init__(self, *, author_id: int, star_shop: bool):
        self.author_id = author_id
        self.star_shop = star_shop

        items_map = STAR_SHOP_ITEMS if star_shop else COIN_SHOP_ITEMS
        stock_key = "star_shop" if star_shop else "coin_shop"
        stock = ensure_shop_stock(load_shop_stock())
        symbol = "✦ stars" if star_shop else "coins"

        options: list[discord.SelectOption] = []
        for name in sorted(items_map.keys(), key=lambda k: items_map[k]["price"]):
            qty = int(stock[stock_key].get(name, 0))
            price = items_map[name]["price"]
            options.append(
                discord.SelectOption(
                    label=f"{name}"[:100],
                    description=f"{price} {symbol} · in stock: {qty}"[:100],
                    value=name,
                    emoji="🟢" if qty > 0 else "⛔",
                )
            )
        ...
        if not options:
            options.append(discord.SelectOption(label="Nothing here", value="__empty__", emoji="⛔"))

        placeholder = "Choose a star-shop item to buy…" if star_shop else "Choose a coin-shop item to buy…"
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.author_id:
            return await interaction.response.send_message(
                embed=error("Shop", "Only the command author can buy from this menu."),
                ephemeral=True,
            )
        choice = self.values[0]
        if choice == "__empty__":
            return await interaction.response.send_message(
                embed=warn("Shop", "Nothing is in stock right now. Check back after restock."),
                ephemeral=True,
            )
        ok, message = _buy_item(self.author_id, choice, star_shop=self.star_shop)
        builder = success if ok else warn
        await interaction.response.send_message(embed=builder("Shop", message), ephemeral=True)


class BuyShopView(discord.ui.View):
    def __init__(self, *, author_id: int, star_shop: bool):
        super().__init__(timeout=120)
        self.author_id = author_id
        self.add_item(_BuyShopSelect(author_id=author_id, star_shop=star_shop))
...
class _ClaimSelect(discord.ui.Select):
    def __init__(self, *, author_id: int, user_inv: dict):
        self.author_id = author_id
        options: list[discord.SelectOption] = []
        for name in sorted(user_inv.keys()):
            qty = int(user_inv.get(name, 0))
            if qty <= 0:
                continue
            if name == "Crash token":
                desc = "Use /claimcrash <stock> — needs a stock name"
            elif name == "Fwiz's USB":
                desc = "Use /claimusb <user> <stock> — needs a target"
            else:
                desc = f"Owned: {qty}"
            options.append(discord.SelectOption(label=name[:100], description=desc[:100], value=name))

        if not options:
            options.append(discord.SelectOption(label="Nothing to claim", value="__empty__", emoji="📭"))

        super().__init__(
            placeholder="Pick an item to claim…",
            min_values=1,
            max_values=1,
            options=options,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.author_id:
            return await interaction.response.send_message(
                embed=error("Claim", "Only the command author can claim from this menu."),
                ephemeral=True,
            )
        choice = self.values[0]
        if choice == "__empty__":
            return await interaction.response.send_message(
                embed=warn("Claim", "Your inventory is empty."), ephemeral=True,
            )
        if choice == "Crash token":
            return await interaction.response.send_message(
                embed=warn("Claim", "Crash tokens need a stock: use `/claimcrash <stock>`."),
                ephemeral=True,
            )
        if choice == "Fwiz's USB":
            return await interaction.response.send_message(
                embed=warn("Claim", "Fwiz's USB needs a target: use `/claimusb <user> <stock>`."),
                ephemeral=True,
            )
        await interaction.response.send_message(
            embed=_embed("Claim", f"Run `/claim {choice}` to confirm using it.", C.SHOP),
            ephemeral=True,
        )
...
class ClaimSelectView(discord.ui.View):
    def __init__(self, *, author_id: int, user_inv: dict):
        super().__init__(timeout=120)
        self.author_id = author_id
        self.add_item(_ClaimSelect(author_id=author_id, user_inv=user_inv))
...
class Shop(commands.Cog):
    ...
    async def shop(self, ctx: commands.Context):
        ...
        embed.set_footer(text="🛍️  Coin Shop  ·  Pick an item below to buy  ·  Restocks every 30 min")
        view = BuyShopView(author_id=ctx.author.id, star_shop=False)
    ...
    async def starshop(self, ctx: commands.Context):
        ...
        embed.set_footer(text="⭐  Star Shop  ·  Pick an item below to buy  ·  Restocks every 30 min")
        view = BuyShopView(author_id=ctx.author.id, star_shop=True)
        await ctx.send(embed=embed, view=view)
    ...
    async def inventory(self, ctx: commands.Context, member: discord.Member = None):
        ...
        # Only show the claim dropdown to the inventory owner viewing themselves.
        if member.id == ctx.author.id:
            embed.set_footer(text="Pick an item below — or use /claim <item> to use it")
            view = ClaimSelectView(author_id=ctx.author.id, user_inv=user_inv)
            await ctx.send(embed=embed, view=view)
        else:
