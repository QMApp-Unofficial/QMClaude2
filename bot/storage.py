...
from config import DATA_DIR, BACKUP_RETENTION
...
def _rotate_backups(stem: str, suffix: str, keep: int):
    """Keep only the `keep` most-recent backups for a given file stem."""
    if keep <= 0:
        return
    try:
        matches = sorted(
            BACKUP_DIR.glob(f"{stem}.*{suffix}.bak"),
...
def _save_json(path: Path, obj: Any, skip_backup: bool = False):
    """
    Atomic save so Railway crashes cannot corrupt files.
    Optionally keeps a rotated backup copy of the previous version.

    skip_backup=True disables the backup write entirely — use for high-churn
    files (e.g. channel_logs) where per-write backups would blow up disk I/O.
    """
    temp = path.with_suffix(path.suffix + ".tmp")

    if path.exists() and not skip_backup and BACKUP_RETENTION > 0:
        try:
            ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
            backup_path = BACKUP_DIR / f"{path.stem}.{ts}{path.suffix}.bak"
            shutil.copy2(path, backup_path)
            _rotate_backups(path.stem, path.suffix, BACKUP_RETENTION)
        except Exception:
            pass
