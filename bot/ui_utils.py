...
class ConfirmView(discord.ui.View):
    """
    Generic yes/no confirmation view. Restricted to the invoking user.
    `.value` is True if confirmed, False if cancelled, None if it timed out.

    Usage:
        view = ConfirmView(ctx.author.id)
        msg = await ctx.send(embed=warn("Confirm", "Proceed?"), view=view)
        await view.wait()
        if view.value:
            ...
    """
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message(
                embed=error("Not for you", "Only the command author can respond here."),
                ephemeral=True,
            )
            return False
        return True
    ...
    async def _confirm(self, interaction: discord.Interaction, _button: discord.ui.Button):
        self.value = True
        self._disable_all()
    ...
    async def _cancel(self, interaction: discord.Interaction, _button: discord.ui.Button):
        self.value = False
        self._disable_all()
        await interaction.response.edit_message(view=self)
        self.stop()
